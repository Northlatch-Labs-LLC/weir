...
export function normaliseType(value) {
    ...
    if (open === -1)
        return normaliseTypeHead(trimmed);
    if (!trimmed.endsWith('>'))
        return null;
 ...
 */
function normaliseTypeHead(value) {
    const trimmed = value.trim();
    if (trimmed === '')
        return null;
    ...
    if (normalisedAddress === null)
        return null;
    if (!IDENTIFIER.test(moduleName) || !IDENTIFIER.test(typeName))
        return null;
 ...
 */
export function normaliseTarget(value) {
 ...
 */
function splitTypeParameters(inner) {
    const out = [];
    let depth = 0;
    let start = 0;
