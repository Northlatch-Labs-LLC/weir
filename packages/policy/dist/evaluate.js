 ...
 */
export function evaluate(effects, policy, ledger) {
 ...
 */
export function evaluateWith(rules, effects, policy, ledger) {
    const input = { effects, policy, ledger };
    for (const rule of rules) {
        const reason = rule.check(input);
        if (reason !== null) {
            const decision = { allow: false, reason: `[${rule.id}] ${reason}`, ruleId: rule.id };
...
export function rulesWithout(id) {
