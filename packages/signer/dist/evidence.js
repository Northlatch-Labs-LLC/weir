 ...
 */
import { classify, decodeAbort, fail, ok } from '@projectx-social/sdk';
 ...
 */
export function grpcSimulation(client) {
    return {
 ...
 */
export function readSimulation(response, sender) {
    const source = 'simulateTransaction';
    ...
    if (payload === null) {
        return fail('malformed', source, 'the response carried neither a Transaction nor a FailedTransaction, so it could not be ' +
            'shown to have succeeded. Nothing was signed. This is a client/server shape mismatch.');
    }
    const status = asObject(payload['status']);
    if (status === null) {
        return fail('malformed', source, 'the simulation carried no status field, so it could not be shown to have succeeded. ' +
    ...
    if (wouldSucceed !== (succeeded !== null)) {
        ...
        return fail('malformed', source, `the response is a ${succeeded !== null ? 'Transaction' : 'FailedTransaction'} but its ` +
...
function readEffects(payload, sender) {
    ...
    if (balanceChangesObserved) {
        for (const item of rawChanges) {
            ...
            if (typeof coinType !== 'string' ||
                ...
                return fail('malformed', source, 'a balance change did not carry string coinType, address and amount. Amounts are not ' +
    ...
    if (transactionData === null) {
        return fail('malformed', source, 'the response carried no parsed transaction data, so the commands, the gas budget and the ' +
    ...
    const gasBudgetMist = typeof gasData?.['budget'] === 'string'
        ? gasData['budget']
        : // A missing budget is reported as a string no rule can parse, so `gas-budget` refuses.
            'unreadable';
    const reportedSender = typeof transactionData['sender'] === 'string' ? transactionData['sender'] : sender;
    const inputs = Array.isArray(transactionData['inputs']) ? transactionData['inputs'] : [];
    ...
    for (let index = 0; index < commands.length; index += 1) {
        ...
        const kind = typeof command['$kind'] === 'string' ? command['$kind'] : 'Unknown';
        ...
        if (kind === 'MoveCall') {
            ...
            const typeArguments = Array.isArray(call['typeArguments'])
                ? call['typeArguments'].map((t) => (typeof t === 'string' ? t : 'unreadable'))
                : [];
            moveCalls.push({
                index,
 ...
 */
function collectInputReferences(value, commandIndex, into, depth = 0) {
    if (depth > MAX_ARGUMENT_DEPTH)
        return;
    if (Array.isArray(value)) {
        for (const item of value)
            collectInputReferences(item, commandIndex, into, depth + 1);
        return;
    }
    ...
    if (object['$kind'] === 'Input' && typeof object['Input'] === 'number') {
        ...
        if (Number.isInteger(index)) {
            ...
            if (seen === undefined)
                into.set(index, [commandIndex]);
            else if (!seen.includes(commandIndex))
                seen.push(commandIndex);
        }
        return;
    }
 ...
 */
function readObjectInputs(inputs, references) {
    const out = [];
...
function classifyInput(index, raw, commandIndexes) {
    const unclassified = (objectId) => ({
        index,
        objectId,
    ...
    if (kind === 'Pure' || kind === 'UnresolvedPure')
        return null;
    if (kind !== 'Object') {
        /*
 ...
 */
function resolveAddressArgument(argument, inputs) {
    ...
    const input = asObject(inputs[arg['Input']]);
    if (input === null || input['$kind'] !== 'Pure')
        return UNRESOLVED_RECIPIENT;
...
function stringifyError(error) {
    ...
    if (error === null || error === undefined)
        return 'failed with no error detail';
    ...
    if (object !== null && typeof object['message'] === 'string')
...
export async function buildBytes(client, transaction, sender) {
    ...
    try {
        ...
        */
        if (/MoveAbort|abort code:/i.test(detail)) {
            ...
            return fail('malformed', source, `the transaction aborts and could not even be built: ${detail}` +
