...
export function readOnlySigner(options) {
    const source = `read-only signer ${options.address}`;
    const because = options.because ?? 'no signing key is configured for this address.';
    const refuse = () => Promise.resolve(fail('unconfigured', source, `${because} Nothing was signed and nothing was submitted. This is a deliberate absence ` +
