 ...
 */
function fromKeypair(keypair, scheme) {
 ...
 */
export function localKeypairSignerFromSecret(secret) {
    ...
    try {
        ...
        return fail('unconfigured', source, 'the signing secret could not be decoded as a bech32 Sui private key (suiprivkey1…). ' +
...
export async function localKeypairSignerFromKeystore(args) {
    ...
    for (const entry of entries) {
        ...
        if (!candidate.ok)
            continue;
        if (normalise(candidate.value.address) === wanted)
            return candidate;
    }
    return fail('not-found', source, `holds no key for ${wanted}. Nothing about the other entries is reported by design.`);
}
/**
 ...
 */
const FLAG_TO_SCHEME = {
    0x00: 'ed25519',
    0x02: 'secp256r1',
};
function fromParsed(scheme, secretKey, source) {
    try {
    ...
    return fail('unconfigured', source, `signature scheme ${JSON.stringify(scheme)} is not supported by LocalKeypairSigner. ` +
