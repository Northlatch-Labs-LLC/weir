 ...
 */
import { fail, ok, simulate } from '@projectx-social/sdk';
import { canonicalPolicyJson, evaluate, } from '@projectx-social/policy';
import { AuditLog, policyHash } from './audit.js';
import { buildBytes, grpcSimulation } from './evidence.js';
export function policySigner(options) {
    ...
    const refuse = (kind, reason, txDigest) => {
        audit.append({
            ts: Date.now(),
            address,
            txDigest,
        ...
        return fail(kind, source, reason);
    };
    /**
     ...
     */
    const refuseUnexpected = (error) => {
        const detail = error instanceof Error ? error.message : String(error);
        const reason = `refused: the gate raised an unexpected error and nothing was signed — ${detail}. This is ` +
        ...
        try {
            return refuse('malformed', reason, '');
        }
        catch {
            ...
            return fail('malformed', source, reason);
        }
    };
    /**
     ...
     */
    const attemptSign = async (transaction) => {
        ...
        if (!observed.ok) {
            return refuse(observed.failure.kind, `the simulation could not be read, so nothing was signed: ${observed.failure.detail}`, '');
        }
        const evidence = observed.value;
        if (!evidence.wouldSucceed) {
            return refuse('malformed', `the transaction would abort, so it was not signed: ${explainAbort(evidence.abort, evidence.status)}`, evidence.txDigest);
        }
        ...
        if (!verdict.ok) {
            return refuse(verdict.failure.kind, `the SDK simulation gate did not confirm success (${verdict.failure.kind}): ` +
                ...
                `returned under FailedTransaction and simulate() reads only Transaction.`, evidence.txDigest);
        }
        if (!verdict.value.wouldSucceed) {
            return refuse('malformed', `the SDK simulation gate reports the transaction would fail: ${verdict.value.status}`, evidence.txDigest);
        }
        // 4. Policy.
        const decision = evaluate(evidence.effects, options.policy, options.ledger());
        if (!decision.allow) {
        ...
        if (!signature.ok) {
            ...
            return refuse(signature.failure.kind, `the policy permitted this transaction and the signer could not produce a signature: ` +
                `${signature.failure.detail}`, evidence.txDigest);
        }
        return ok({
    ...
    return {
        ...
        signPersonalMessage: async (bytes) => {
            ...
            try {
                ...
                if (!signature.ok) {
                    return refuse(signature.failure.kind, `a personal message could not be signed: ${signature.failure.detail}`, '');
                }
                audit.append({
                    ts: Date.now(),
                    address,
...
function explainAbort(abort, status) {
    if (abort === undefined)
        return status;
...
//# sourceMappingURL=policy-signer.js.map