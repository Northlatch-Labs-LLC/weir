 ...
 */
export function agentKeyFromSecret(secret) {
    ...
    try {
        ...
        void error;
        return fail('unconfigured', source, 'the signing secret could not be decoded as a bech32 Ed25519 Sui private key ' +
 ...
 */
export function agentKeyFromEnv(env) {
    const value = env[AGENT_ENV.secret];
    if (value === undefined || value.trim() === '') {
        return fail('unconfigured', 'agent key', `${AGENT_ENV.secret} is not set. An agent signs as itself and there is no default key — ` +
