 ...
 */
export const BEARER_FIELDS = ['token'];
/**
...
export async function openSession(input) {
    ...
    try {
        ...
        return fail('transport', source, `could not reach ${input.baseUrl}/api/session: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    const expiresAtMs = typeof body?.['expiresAtMs'] === 'number' && Number.isFinite(body['expiresAtMs'])
        ? body['expiresAtMs']
        : null;
    ...
    if (cookie !== null) {
        return ok(credential('cookie', { cookie: `${READ_SESSION_COOKIE}=${cookie}` }, address, expiresAtMs));
    }
    return fail('malformed', source, `the session endpoint returned 200 but carried neither a ${READ_SESSION_COOKIE} cookie nor a ` +
...
function credential(kind, headers, address, expiresAtMs) {
    return {
        kind,
        address,
        expiresAtMs,
        ...
        headers: () => ({ ...headers }),
        isExpired: (nowMs = Date.now()) => expiresAtMs !== null && nowMs >= expiresAtMs,
    };
}
/**
 ...
 */
export function readSessionCookieFrom(headers) {
    ...
    for (const line of all) {
        ...
        if (separator === -1)
            continue;
        if (pair.slice(0, separator).trim() !== READ_SESSION_COOKIE)
            continue;
