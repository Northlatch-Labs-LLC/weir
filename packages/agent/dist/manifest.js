 ...
 */
export const DEFAULT_GAS_BUDGET_MIST = 500000000n;
/**
 ...
 */
export function loadAgentManifest(env, overrides) {
    ...
    if (coinType === undefined || coinType === '') {
        return fail('unconfigured', source, `${AGENT_ENV.coinType} is not set. Every spending call this agent makes is generic over a ` +
    ...
    if (!COIN_TYPE.test(coinType)) {
        return fail('unconfigured', source, `${AGENT_ENV.coinType} is "${coinType}", which is not a fully-qualified Move coin type ` +
            `(expected 0x…::module::Struct).`);
    }
    const rawBaseUrl = env[AGENT_ENV.baseUrl]?.trim();
    if (rawBaseUrl === undefined || rawBaseUrl === '') {
        return fail('unconfigured', source, `${AGENT_ENV.baseUrl} is not set. Feeds, publishing and messaging are HTTP calls against a ` +
            `weir deployment; the chain does not hold them.`);
    }
    const baseUrl = normaliseBaseUrl(rawBaseUrl);
    if (baseUrl === null) {
        return fail('unconfigured', source, `${AGENT_ENV.baseUrl} is "${rawBaseUrl}"; expected an http(s) origin.`);
    }
...
export function isCoinType(value) {
