...
import { approvalBytes, approveSubscription, approveUnlock, createClient, periodIdentity, sealId, sealPackageId, unlockIdentity, } from '@projectx-social/sdk';
...
export class SealHashMismatchError extends Error {
    ...
    expected;
    actual;
    name = 'SealHashMismatchError';
    constructor(blobId, expected, actual) {
        super(`the bytes returned for blob ${blobId} do not match the hash recorded at publish ` +
...
export function identityForApproval(approval) {
 ...
 */
export function approvalTransactionFor(config, approval) {
...
export function openBlob(input) {
    ...
    return new Uint8Array(Buffer.concat([decipher.update(input.ciphertext.subarray(0, split)), decipher.final()]));
}
/**
 ...
 */
export function looksLikeSettling(error) {
...
function base64(value, field, expected) {
    ...
    */
    if (bytes.length === 0)
        throw new Error(`${field} decoded to nothing; it is not base64`);
...
export class SealDecryptor {
    ...
    async decrypt(ref) {
        ...
        */
        ...
        if (sealedTo !== asked) {
            throw new Error(`this content is sealed to identity ${sealedTo} and the approval offered covers ` +
        ...
        if (key === undefined)
            throw last ?? new Error('the key could not be recovered');
     ...
     */
    async approvalBytesFor(approval) {
    ...
    async recoverKeyFromCommittee(input) {
        ...
        if (seal === undefined) {
            throw new Error('this decryptor was built with no key server committee and no recoverKey seam, so it ' +
    ...
    async #fetchBlob(blobId) {
        ...
        throw new Error(`no aggregator served blob ${blobId} — its storage lease may have expired. ` +
            `Tried: ${failures.join('; ')}`);
    }
}
//# sourceMappingURL=seal-node.js.map