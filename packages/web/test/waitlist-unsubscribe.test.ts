...
// codebase. These tests pin the three things that make it true: the link works, it works twice, and
// it tells nobody anything about whose address it names.
...
describe('the route takes a real link off the list', () => {
  ...
  it('answers identically the second time, and the tenth', async () => {
    /*
      Idempotence is not a nicety here. A second click, a forwarded link and a mail client that
      prefetches all arrive at a row that is already gone, and every one of them means the same
      thing to the person who sent the request.
    */
    ...
    query.mockResolvedValueOnce({ rows: [], rowCount: 0 });
    const third = await get(link(token));
  ...
  it('does the same work for the one-click POST a mail provider sends', async () => {
    const token = mintUnsubscribeToken(ADDRESS, SECRET);
    const response = await post(link(token));
...
describe('the route refuses anything it did not sign, before it touches the database', () => {
  ...
  it('refuses when the deployment has no list, without claiming a removal', async () => {
    delete process.env['PROJECTX_DATABASE_URL'];
    const response = await get(link(mintUnsubscribeToken(ADDRESS, SECRET)));

    expect(response.status).toBe(503);
    expect(query).not.toHaveBeenCalled();
    expect(await response.text()).toContain('could not take you off');
  });
