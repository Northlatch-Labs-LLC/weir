...
import {
  ...
  type Guarantee,
  type OnlyChain,
} from '@/components/app/SecurityScreen';
...
const ONLY_CHAIN: readonly OnlyChain[] = [
...
export async function SecurityData({
  ...
  const CMP_ROWS: readonly CompareRow[] = [
  ...
  const CMP_SUMMARY: readonly CompareCard[] = [
    {
      name: 'Weir',
      ours: true,
      figure: feePercent,
      unread: 'reading from the chain',
      note:
        feePercent === null
    ...
    { name: 'Patreon', ours: false, figure: '13 to 16%', unread: '', note: 'A 10% platform fee plus payment processing. The all-in figure is the one a creator actually feels.' },
    { name: 'OnlyFans', ours: false, figure: '20%', unread: '', note: "The platform's published rate, before any payout hold. Flat, and the highest of the three." },
  ];
  ...
  const rows: { key: string; name: string; icon: string; id: string | null; what: string }[] = [
    { key: 'package', name: 'Weir package (running)', icon: 'cube', id: config.ok ? config.value.latestPackageId : null, what: 'The code that executes: subscriptions, unlocks, tier objects, the settlement that takes the platform fee in the same transaction, and the key registry. This is the package to read.' },
    { key: 'origin', name: 'Original publication', icon: 'layers', id: config.ok ? config.value.packageId : null, what: 'Where these types were first published. Move binds type identity to that address forever, so every object Weir has ever made carries this id in its type tag: it is how you recognise one as ours. It is not the code that runs today.' },
    { key: 'keys', name: 'Key registry', icon: 'key', id: keyRegistry, what: 'The published encryption key for each address, used for direct messages between people. This module was added in the upgrade, which is why the original package above does not contain it.' },
    { key: 'platform', name: 'Platform object', icon: 'vault', id: config.ok ? config.value.platformId : null, what: 'Holds the live economic terms: the fee, the referral share, the treasury. Every figure this site prints about fees is read from here.' },
    { key: 'registry', name: 'Account registry', icon: 'layers', id: config.ok ? config.value.registryId : null, what: 'Maps handles to addresses. This is what decides whether a name is free, and it is the same table the mint transaction writes to.' },
  ];

  const contracts: readonly ContractRow[] = rows.map((row) => {
