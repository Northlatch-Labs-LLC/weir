          ...
          <div className="weir-page" style={{ maxWidth: '72rem', marginInline: 'auto', padding: '3rem 1.5rem 4rem' }}>
            ...
            <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,19rem),1fr))' }}>
              {(guarantees ?? []).map((g, i) => (<Fragment key={i}>
                <article className="dh-b3237c6c" data-reveal style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.78),rgba(var(--pb,9,32,42),0.88))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.14)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.07),0 14px 34px -26px rgba(var(--shade-rgb,0,0,0),0.85)', padding: '1.5rem', transition: 'border-color 0.18s ease,transform 0.18s ease,box-shadow 0.18s ease' }}>
                  ...
                  <p style={{ margin: '1.25rem 0 0', paddingTop: '1rem', borderTop: '1px solid rgba(var(--line-rgb,28,61,71),0.9)', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', textWrap: 'pretty' }}>{g.mechanism}</p>
                </article>
              </Fragment>))}
            </div>

            <section data-reveal aria-labelledby="only-title" style={{ marginTop: '4rem', textAlign: 'center' }}>
              <p style={{ margin: '0 0 0.875rem', textAlign: 'center', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>Why this cannot be copied</p>
              <h2 id="only-title" style={{ margin: '0 auto 2rem', maxWidth: '36ch', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', lineHeight: '1.1', letterSpacing: '-0.032em', fontSize: 'clamp(1.75rem,1.2rem + 1.8vw,2.5rem)', textWrap: 'balance' }}>Three things <span style={{ background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd) 46%,var(--sand,#d9c9a3))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent', filter: 'drop-shadow(0 0 26px rgba(var(--crest-rgb,139,227,198),0.35))' }}>only a chain can do</span></h2>
              <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,18rem),1fr))', textAlign: 'left' }}>
                {(onlyChain ?? []).map((o, i) => (<Fragment key={i}>
                  <div style={{ background: 'linear-gradient(180deg,rgba(var(--pc,26,66,78),0.88),rgba(var(--pd,11,37,48),0.92))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.22)', borderTop: '2px solid var(--crest,#8be3c6)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.08),0 30px 70px -46px rgba(var(--crest-rgb,139,227,198),0.55)', padding: '1.75rem' }}>
                    <p style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontVariantNumeric: 'tabular-nums', fontSize: '2rem', fontWeight: '500', background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent', filter: 'drop-shadow(0 0 22px rgba(var(--crest-rgb,139,227,198),0.4))' }}>{o.figure}</p>
            ...
            <section data-reveal aria-labelledby="cmp-title" style={{ marginTop: '4rem' }}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ margin: '0 0 0.75rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>Side by side</p>
                <h2 id="cmp-title" style={{ margin: '0 auto 0.75rem', maxWidth: '36ch', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', lineHeight: '1.1', letterSpacing: '-0.032em', fontSize: 'clamp(1.75rem,1.2rem + 1.8vw,2.5rem)', textWrap: 'balance' }}>The same questions, <span style={{ background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd) 46%,var(--sand,#d9c9a3))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent', filter: 'drop-shadow(0 0 26px rgba(var(--crest-rgb,139,227,198),0.35))' }}>asked of everyone</span></h2>
              ...
              <div style={{ display: 'grid', gap: '1.25rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,16rem),1fr))', marginBottom: '2.5rem' }}>
                {(cmpSummary ?? []).map((p, i) => (<Fragment key={i}>
                  <article style={{ position: 'relative', overflow: 'hidden', borderRadius: '10px', padding: '1.75rem', background: `${p.bg}`, border: `1px solid ${p.border}`, boxShadow: `${p.shadow}` }}>
                    ...
                    <p style={{ margin: '0', display: 'flex', alignItems: 'center', gap: '0.5rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: `${p.nameColor}` }}>{p.name}</p>
                    <p style={{ margin: '0.875rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontVariantNumeric: 'tabular-nums', fontSize: '2.25rem', fontWeight: '500', lineHeight: '1', color: `${p.figureColor}`, background: `${p.figureBg}`, WebkitBackgroundClip: `${p.clip}`, backgroundClip: `${p.clip}`, filter: `${p.figureGlow}` }}>{p.figure}</p>
                  ...
                  <article style={{ border: '1px solid rgba(var(--crest-rgb,139,227,198),0.12)', borderRadius: '10px', background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.7),rgba(var(--pb,9,32,42),0.85))', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.05)', padding: '1.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gap: '0.75rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,14rem),1fr))' }}>
                      <div style={{ borderRadius: '10px', padding: '1rem 1.125rem', background: 'rgba(var(--crest-rgb,139,227,198),0.08)', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.32)', boxShadow: '0 0 22px -14px rgba(var(--crest-rgb,139,227,198),0.8)' }}>
                        <p style={{ margin: '0 0 0.5rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--crest,#8be3c6)' }}>Weir</p>
                      ...
                      <div style={{ borderRadius: '10px', padding: '1rem 1.125rem', background: 'rgba(var(--pe,4,22,29),0.5)', border: '1px solid rgba(var(--line-rgb,28,61,71),0.9)' }}>
                        <p style={{ margin: '0 0 0.5rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>Patreon</p>
                        <p style={{ margin: '0', fontSize: '0.9375rem', lineHeight: '1.55', color: 'var(--dim,#a3bcb8)', textWrap: 'pretty' }}>{row.patreon}</p>
                      </div>
                      <div style={{ borderRadius: '10px', padding: '1rem 1.125rem', background: 'rgba(var(--pe,4,22,29),0.5)', border: '1px solid rgba(var(--line-rgb,28,61,71),0.9)' }}>
                        <p style={{ margin: '0 0 0.5rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>OnlyFans</p>
              ...
              <div style={{ marginTop: '1.5rem', border: '1px solid rgba(var(--line-rgb,28,61,71),0.9)', borderLeft: '3px solid var(--sand,#d9c9a3)', borderRadius: '10px', padding: '1.25rem 1.5rem' }}>
                <p style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>Sources</p>
                <ul style={{ margin: '0.75rem 0 0', padding: '0', listStyle: 'none', display: 'grid', gap: '0.5rem' }}>
                  {(cmpSources ?? []).map((src, i) => (<Fragment key={i}>
                    <li style={{ fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', lineHeight: '1.6', color: 'var(--dim,#a3bcb8)', textWrap: 'pretty' }}>{src}</li>
                  </Fragment>))}
                </ul>
              </div>
            </section>
                  ...
                  <article className="dh-ccd4b338" style={{ position: 'relative', overflow: 'hidden', display: 'grid', gap: '1.25rem', gridTemplateColumns: 'minmax(0,1fr) auto', alignItems: 'center', background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.78),rgba(var(--pb,9,32,42),0.88))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.14)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.07),0 14px 34px -26px rgba(var(--shade-rgb,0,0,0),0.85)', padding: '1.5rem', transition: 'border-color 0.18s ease,box-shadow 0.18s ease' }}>
                    ...
                    <div style={{ minWidth: '0', paddingLeft: '0.5rem' }}>
                      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <span style={{ fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.15rem 0.55rem', borderRadius: '99px', border: `1px solid ${c.tagBorder}`, color: `${c.tagColor}` }}>{c.tag}</span>
                      </div>
                      <p style={{ margin: '0.75rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', wordBreak: 'break-all', color: `${c.idColor}`, fontStyle: `${c.idStyle}` }}>{c.id}</p>
                    ...
                    <div style={{ display: 'grid', gap: '0.5rem', justifyItems: 'stretch' }}>
                      ...
                      <a className="dh-640f211f" href={c.href} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '0.45rem', padding: '0.55rem 1rem', borderRadius: '10px', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', textDecoration: 'none', border: '1px solid rgba(var(--line-rgb,28,61,71),0.9)', color: `${c.linkColor}`, transition: 'border-color 0.12s ease,color 0.12s ease' }}>{c.linkLabel}</a>
                    </div>
                  </article>
                </Fragment>))}
              </div>
            ...
            <div data-reveal style={{ marginTop: '3rem', border: '1px solid rgba(var(--alert-rgb,242,162,155),0.3)', borderLeft: '3px solid var(--alert,#f2a29b)', borderRadius: '10px', padding: '1.5rem', maxWidth: '72ch' }}>
              <p style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--alert,#f2a29b)' }}>What we do not claim</p>
              <p style={{ margin: '0.625rem 0 0', color: 'var(--dim,#a3bcb8)', fontSize: '0.9375rem', textWrap: 'pretty' }}>A chain does not make software correct. Move code can be wrong, a validator can misbehave, staking yield is variable and low single digits, and a key you lose is a key nobody can restore for you.</p>
            </div>
          </div>
    </>
  );
