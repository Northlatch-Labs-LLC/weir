  ...
  return (
    <>
          <div className="weir-page" style={{ maxWidth: '72rem', marginInline: 'auto', padding: '3rem 1.5rem 4rem' }}>
              ...
              {(creators ?? []).map((c, i) => (<Fragment key={i}>
                <article style={{ background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.78),rgba(var(--pb,9,32,42),0.88))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.14)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.07),0 14px 34px -26px rgba(var(--shade-rgb,0,0,0),0.85)', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <span aria-hidden="true" style={{ width: '2.5rem', height: '2.5rem', borderRadius: '50%', background: 'var(--line-2,var(--line-2,#123039))', border: '1px solid var(--line,#1c3d47)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', flexShrink: '0' }}>{c.initials}</span>
                    <div style={{ minWidth: '0' }}>
                      <p style={{ margin: '0', fontSize: '1.0625rem', fontWeight: '600', color: 'var(--ink,#dce9e6)' }}>{c.displayName}{c.isAgent === true && <span style={{ marginLeft: '0.5rem', fontFamily: 'var(--weir-mono)', fontSize: '0.7rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--crest,#8be3c6)', border: '1px solid var(--line,#1c3d47)', borderRadius: '3px', padding: '0.1rem 0.35rem', verticalAlign: 'middle' }}>Declared agent</span>}</p>
                      <p style={{ margin: '0', fontFamily: 'var(--weir-mono)', fontWeight: '500', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)' }}>@{c.handle}</p>
                    </div>
                  </div>
                  ...
                  */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: 'auto auto', gridAutoFlow: 'column', gap: '0.25rem 1rem', paddingTop: '1rem', background: 'linear-gradient(to right,var(--crest,#8be3c6) 0 24px,var(--line,#1c3d47) 24px) top left / 100% 1px no-repeat' }}>
                    <div style={{ display: 'contents' }}>
                      <span style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>Pooled</span>
                    ...
                    <div style={{ display: 'contents' }}>
                      <span style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>Yield shared back</span>
            ...
            <p style={{ margin: '2rem 0 0', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>{creatorCount}</p>
          </div>
    </>
  );
}
