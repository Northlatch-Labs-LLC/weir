    ...
    <>
          <div className="weir-page" style={{ maxWidth: '52rem', marginInline: 'auto', padding: '3rem 1.5rem 6rem' }}>
            <section aria-label="Vault owner" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '1rem' }}>
              <span aria-hidden="true" style={{ width: '3.5rem', height: '3.5rem', borderRadius: '50%', background: 'var(--line-2,#123039)', border: '1px solid var(--line,#1c3d47)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: '\'Geist Mono\',monospace', fontSize: '1.0625rem', color: 'var(--crest,#8be3c6)', flexShrink: '0', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.08)' }}>{myInitials}</span>
              <div style={{ minWidth: '0' }}>
                <p style={{ margin: '0 0 0.2rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.06em', color: 'var(--crest,#8be3c6)' }}>{myHandle}</p>
            ...
            <section aria-label="On-chain object" style={{ background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.78),rgba(var(--pb,9,32,42),0.88))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.14)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.07),0 14px 34px -26px rgba(var(--shade-rgb,0,0,0),0.85)', padding: '1.5rem' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', justifyContent: 'space-between', gap: '0.75rem' }}>
                ...
                <a href={vaultScanHref} style={{ fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem' }}>{vaultScanLabel}</a>
              </div>
              <p style={{ margin: '1.25rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', wordBreak: 'break-all', color: 'var(--crest,#8be3c6)' }}>{vaultType}</p>
              <p style={{ margin: '0.25rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>version {vaultVersion}</p>
              <dl style={{ margin: '1.5rem 0 0', display: 'grid', gap: '0.5rem' }}>
                {(vaultFields ?? []).map((f, i) => (<Fragment key={i}>
                  <div className="weir-kv" style={{ display: 'grid', gridTemplateColumns: '12rem minmax(0,1fr)', gap: '0.75rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--line,#1c3d47)' }}>
                    <dt style={{ fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>{f.k}</dt>
                    <dd style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', wordBreak: 'break-all', color: 'var(--ink,#dce9e6)' }}>{f.v}</dd>
                  </div>
                </Fragment>))}
              </dl>
            ...
            {members !== null && (
              <section aria-label="Community members" style={{ marginTop: '1.5rem', background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.78),rgba(var(--pb,9,32,42),0.88))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.14)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.07),0 14px 34px -26px rgba(var(--shade-rgb,0,0,0),0.85)', padding: '1.5rem' }}>
                ...
                */}
                <p style={{ margin: '0.5rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: members.reconciles ? 'var(--dim,#a3bcb8)' : 'var(--alert,#f2a29b)', textWrap: 'pretty' }}>{members.summary}</p>
                {members.rows.length > 0 && (
                  <div style={{ overflowX: 'auto', marginTop: '1.25rem' }}>
                    <table className="weir-members" style={{ width: '100%', borderCollapse: 'collapse', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem' }}>
                      <thead>
                        <tr>
                ...
                <p style={{ margin: '1.25rem 0 0', maxWidth: '62ch', textWrap: 'pretty', color: 'var(--dim,#a3bcb8)', fontSize: '0.9375rem' }}>Read from the vault's positions table on every load. "Claimable now" is the contract's own claimable_rebate arithmetic applied to the current accumulator, so a harvest that landed a minute ago is already in it.</p>
              </section>
            )}
            ...
            <div style={{ marginTop: '1.5rem', border: '1px solid var(--line,#1c3d47)', borderLeft: '3px solid var(--sand,#d9c9a3)', borderRadius: '10px', padding: '1.25rem 1.5rem', maxWidth: '62ch' }}>
              <p style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>Deposit & withdraw</p>
