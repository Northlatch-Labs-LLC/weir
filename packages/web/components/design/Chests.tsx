          ...
          <div className="weir-page" style={{ maxWidth: '72rem', marginInline: 'auto', padding: '3rem 1.5rem 4rem' }}>
              ...
              {(withHandlers ?? []).map((c, i) => (<Fragment key={i}>
                <article className="dh-0b27bfb9" data-reveal style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.78),rgba(var(--pb,9,32,42),0.88))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.14)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.07),0 14px 34px -26px rgba(var(--shade-rgb,0,0,0),0.85)', padding: '1.5rem', transition: 'border-color 0.18s ease,transform 0.18s ease,box-shadow 0.18s ease' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '0.75rem' }}>
                    ...
                    <span style={{ fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.06em', padding: '0.15rem 0.55rem', borderRadius: '99px', border: `1px solid ${c.tagBorder}`, color: `${c.tagColor}`, whiteSpace: 'nowrap' }}>{c.tag}</span>
                  </div>
                  <p style={{ margin: '0.75rem 0 0', color: 'var(--dim,#a3bcb8)', fontSize: '0.9375rem', maxWidth: '62ch', textWrap: 'pretty' }}>{c.body}</p>
                  <p style={{ margin: '1.5rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>In the chest</p>
                  ...
                  <p style={{ margin: '0.2rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', letterSpacing: '0.04em', color: 'var(--dim,#a3bcb8)' }}>{c.potNote}</p>
                  <p style={{ margin: '0.5rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>{c.split}</p>
                  <div style={{ marginTop: '1.25rem', display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                    {(c.amounts ?? []).map((amt, i) => (<Fragment key={i}>
                      <button type="button" onClick={amt.onClick} style={{ padding: '0.45rem 0.9rem', borderRadius: '99px', cursor: 'pointer', fontFamily: '\'Geist Mono\',monospace', fontVariantNumeric: 'tabular-nums', fontSize: '0.8125rem', background: `${amt.bg}`, color: `${amt.color}`, border: `1px solid ${amt.border}`, transition: 'border-color 0.12s ease,color 0.12s ease,background-color 0.12s ease' }}>{amt.label}</button>
                    </Fragment>))}
                  </div>
                  ...
                  <p style={{ margin: '0.75rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: `${c.noteColor}` }}>{c.note}</p>
                </article>
              </Fragment>))}
            </div>
            ...
            */}
            <section data-reveal aria-labelledby="ways-title" style={{ marginTop: '3.5rem' }}>
              <p style={{ margin: '0 0 0.625rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>Before you give</p>
              <h2 id="ways-title" style={{ margin: '0', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', fontSize: 'clamp(1.35rem,1.1rem + 0.8vw,1.75rem)', letterSpacing: '-0.03em' }}>Three ways to back someone. <span style={{ background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd) 46%,var(--sand,#d9c9a3))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>Only one is a gift.</span></h2>
                  ...
                  <thead>
                    <tr>
                      {COMPARE_COLS.map((h, i) => (
                        <th key={i} scope="col" style={{ textAlign: i === 0 ? 'left' : 'left', padding: '0.875rem 1rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.75rem', fontWeight: '500', letterSpacing: '0.12em', textTransform: 'uppercase', color: i === 1 ? 'var(--crest,#8be3c6)' : 'var(--sand,#d9c9a3)', borderBottom: '1px solid var(--line,#1c3d47)', background: i === 1 ? 'rgba(var(--crest-rgb,139,227,198),0.05)' : 'transparent' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={r}>
                        {row.map((cell, c) => (
                          <td key={c} data-label={c === 0 ? undefined : COMPARE_COLS[c]} style={{ padding: '0.875rem 1rem', borderBottom: r === 3 ? 'none' : '1px solid rgba(var(--line-rgb,28,61,71),0.6)', color: c === 0 ? 'var(--dim,#a3bcb8)' : 'var(--ink-2,#b9cdc9)', fontFamily: c === 0 ? '\'Geist Mono\',monospace' : 'inherit', fontSize: c === 0 ? '0.8125rem' : '0.9375rem', letterSpacing: c === 0 ? '0.04em' : 'normal', background: c === 1 ? 'rgba(var(--crest-rgb,139,227,198),0.05)' : 'transparent', whiteSpace: c === 0 ? 'nowrap' : 'normal' }}>{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
            ...
            */}
            <section data-reveal aria-labelledby="how-title" style={{ marginTop: '3.5rem' }}>
              <p style={{ margin: '0 0 0.625rem', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>What happens</p>
              ...
              <ol style={{ margin: '1.5rem 0 0', padding: '0', listStyle: 'none', display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,15rem),1fr))' }}>
                {[
                  ...
                  <li key={i} style={{ background: 'linear-gradient(180deg,rgba(var(--pa,20,52,62),0.6),rgba(var(--pb,9,32,42),0.8))', border: '1px solid var(--line,#1c3d47)', borderRadius: '10px', padding: '1.25rem' }}>
                    <span aria-hidden="true" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '1.75rem', height: '1.75rem', borderRadius: '8px', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.35)', background: 'rgba(var(--crest-rgb,139,227,198),0.08)', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)' }}>{s.n}</span>
            ...
            <section data-reveal aria-labelledby="standing-title" style={{ marginTop: '3rem', background: 'linear-gradient(180deg,rgba(var(--pc,26,66,78),0.88),rgba(var(--pd,11,37,48),0.92))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.22)', borderTop: '2px solid var(--crest,#8be3c6)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.08),0 30px 70px -46px rgba(var(--crest-rgb,139,227,198),0.55)', padding: '2rem' }}>
              <p style={{ margin: '0 0 0.875rem', textAlign: 'center', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>What a donation is</p>
              <h2 id="standing-title" style={{ margin: '0', textAlign: 'center', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', fontSize: '1.5rem', letterSpacing: '-0.03em' }}>A gift, <span style={{ background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd) 46%,var(--sand,#d9c9a3))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent', filter: 'drop-shadow(0 0 26px rgba(var(--crest-rgb,139,227,198),0.35))' }}>priced honestly</span></h2>
              ...
              <div style={{ marginTop: '1.5rem', display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,14rem),1fr))', alignItems: 'end' }}>
                <div>
                  <label htmlFor="chestdep" style={{ display: 'block', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>You give</label>
                  <div style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <input id="chestdep" type="number" min="0" step="50" value={chestDeposit} onChange={onChestDeposit} style={{ width: '9rem', background: 'var(--bg,#04161d)', border: '1px solid var(--line,#1c3d47)', borderRadius: '10px', padding: '0.6rem 0.875rem', color: 'var(--ink,#dce9e6)', fontFamily: '\'Geist Mono\',monospace', fontVariantNumeric: 'tabular-nums', fontSize: '0.9375rem' }}/>
                    <span style={{ fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>SUI</span>
                  </div>
                </div>
                <div>
                  <p style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>They receive</p>
                  <p style={{ margin: '0.25rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontVariantNumeric: 'tabular-nums', fontSize: '1.75rem', fontWeight: '500', background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>{chestWeight}</p>
                </div>
                <div>
                  <p style={{ margin: '0', fontFamily: '\'Geist Mono\',monospace', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>You get back</p>
                  <p style={{ margin: '0.25rem 0 0', fontFamily: '\'Geist Mono\',monospace', fontVariantNumeric: 'tabular-nums', fontSize: '1.75rem', fontWeight: '500', color: 'var(--ink,#dce9e6)' }}>{chestWithdrawable}</p>
                </div>
              </div>
              <div style={{ marginTop: '1.5rem', height: '8px', borderRadius: '99px', background: 'var(--bg,#04161d)', border: '1px solid var(--line,#1c3d47)', overflow: 'hidden' }}>
                <span aria-hidden="true" style={{ display: 'block', height: '100%', width: `${chestWeightPct}`, background: 'linear-gradient(100deg,var(--crest,#8be3c6),var(--teal,#7fd8dd))', transition: 'width 0.5s cubic-bezier(.16,1,.3,1)' }}></span>
              </div>
