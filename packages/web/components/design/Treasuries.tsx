  ...
  return (
    <>
          <div className="weir-page" style={{ maxWidth: '72rem', marginInline: 'auto', padding: '3rem 1.5rem 4rem' }}>
            ...
            <p style={{ margin: '0 0 1rem', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>The pools that fill it</p>
            {(treasuries ?? []).length === 0 ? (
              /* A measured absence, stated as one. A header row over nothing reads as a table that failed to load. */
              <p data-reveal style={{ margin: '0', padding: '1.25rem', border: '1px solid var(--line,#1c3d47)', borderRadius: '10px', background: 'var(--panel,#0b2530)', fontFamily: 'var(--weir-mono)', fontSize: '0.875rem', color: 'var(--dim,#a3bcb8)', textWrap: 'pretty' }}>No pools open yet. The first creator to open one will appear here.</p>
            ) : (
                ...
                <thead>
                  <tr>
                    {(treasuryCols ?? []).map((col, i) => (<Fragment key={i}>
                      <th scope="col" style={{ textAlign: 'left', padding: '0.875rem 1.25rem', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)', borderBottom: '1px solid var(--line,#1c3d47)', whiteSpace: 'nowrap' }}>{col}</th>
                    </Fragment>))}
                  </tr>
                </thead>
                <tbody>
                  {(treasuries ?? []).map((t, i) => (<Fragment key={i}>
                    <tr className="dh-c04cdaa8" style={{ transition: 'background-color 0.12s ease' }}>
                      ...
                      <td data-label={treasuryCols[3]} style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--line,#1c3d47)', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)', whiteSpace: 'nowrap' }}>{t.validator}</td>
            ...
            <section data-reveal aria-labelledby="ladder-title" style={{ marginTop: '3rem', display: 'grid', gap: '3rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,20rem),1fr))', alignItems: 'start' }}>
              <div>
                <p style={{ margin: '0 0 0.75rem', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>The withdrawal ladder</p>
                <h2 id="ladder-title" style={{ margin: '0', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', fontSize: '1.5rem', letterSpacing: '-0.03em' }}>{ladder.length} rungs, so <span className="weir-owned">nobody waits</span></h2>
                ...
                <div style={{ display: 'grid', gap: '0.625rem' }}>
                  {(ladder ?? []).map((l, i) => (<Fragment key={i}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
                      <span style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)', width: '3.5rem', flexShrink: '0' }}>{l.name}</span>
                      ...
                      <span style={{ fontFamily: 'var(--weir-mono)', fontVariantNumeric: 'tabular-nums', fontSize: '0.8125rem', color: `${l.color}`, width: '5.5rem', textAlign: 'right', flexShrink: '0' }}>{l.state}</span>
                    </div>
                  </Fragment>))}
                </div>
                <p style={{ margin: '1.5rem 0 0', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>{epochLabel}</p>
              </div>
            </section>
            ...
            <section data-reveal aria-labelledby="sim-title" style={{ marginTop: '3rem', background: 'linear-gradient(180deg,rgba(var(--pc,26,66,78),0.88),rgba(var(--pd,11,37,48),0.92))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.22)', borderTop: '2px solid var(--crest,#8be3c6)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.08),0 30px 70px -46px rgba(var(--crest-rgb,139,227,198),0.55)', padding: '2rem' }}>
              <p style={{ margin: '0 0 0.75rem', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>Pool simulator</p>
              <h2 id="sim-title" style={{ margin: '0', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', fontSize: '1.5rem', letterSpacing: '-0.03em' }}>What your deposit does, and <span className="weir-owned">what it costs you</span></h2>
              <div style={{ marginTop: '1.5rem', display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,13rem),1fr))' }}>
                <div>
                  <label htmlFor="simamt" style={{ display: 'block', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>Amount</label>
                  <div style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <input id="simamt" type="number" min="0" step="100" value={simAmount} onChange={onSimAmount} style={{ width: '9rem', background: 'var(--bg,#04161d)', border: '1px solid var(--line,#1c3d47)', borderRadius: '10px', padding: '0.6rem 0.875rem', color: 'var(--ink,#dce9e6)', fontFamily: 'var(--weir-mono)', fontVariantNumeric: 'tabular-nums', fontSize: '0.9375rem' }}/>
                    <span style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--dim,#a3bcb8)' }}>SUI</span>
                  </div>
                </div>
                <div>
                  <label htmlFor="simwho" style={{ display: 'block', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>Behind</label>
                  <select id="simwho" value={simWho} onChange={onSimWho} style={{ marginTop: '0.5rem', width: '100%', background: 'var(--bg,#04161d)', border: '1px solid var(--line,#1c3d47)', borderRadius: '10px', padding: '0.6rem 0.875rem', color: 'var(--ink,#dce9e6)', fontFamily: 'var(--weir-mono)', fontSize: '0.9375rem' }}>
                ...
                <div>
                  <p style={{ margin: '0', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)' }}>Stays yours</p>
                  <p style={{ margin: '0.25rem 0 0', fontFamily: 'var(--weir-mono)', fontVariantNumeric: 'tabular-nums', fontSize: '1.75rem', fontWeight: '500', color: 'var(--ink,#dce9e6)' }}>{simPrincipal}</p>
                </div>
              </div>
              <div style={{ marginTop: '1.75rem', display: 'grid', gap: '1rem' }}>
                {(simRows ?? []).map((row, i) => (<Fragment key={i}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem 1.5rem', alignItems: 'baseline', paddingBottom: '0.75rem', borderBottom: '1px solid var(--line,#1c3d47)' }}>
                    <span style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--dim,#a3bcb8)', minWidth: '14rem' }}>{row.label}</span>
            ...
            */}
            <section data-reveal aria-labelledby="share-title" style={{ marginTop: '3rem', background: 'linear-gradient(180deg,rgba(var(--pc,26,66,78),0.88),rgba(var(--pd,11,37,48),0.92))', border: '1px solid rgba(var(--crest-rgb,139,227,198),0.22)', borderTop: '2px solid var(--crest,#8be3c6)', borderRadius: '10px', boxShadow: 'inset 0 1px 0 rgba(var(--hi-rgb,220,233,230),0.08),0 30px 70px -46px rgba(var(--crest-rgb,139,227,198),0.55)', padding: '2rem' }}>
              <p style={{ margin: '0 0 0.75rem', fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', fontWeight: '500', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--sand,#d9c9a3)' }}>The share a creator sets</p>
              <h2 id="share-title" style={{ margin: '0', fontFamily: '\'Geist\',system-ui,sans-serif', fontWeight: '700', fontSize: '1.5rem', letterSpacing: '-0.03em' }}>Some of the yield <span className="weir-owned">can come back to you</span></h2>
              <p style={{ margin: '1rem 0 0', maxWidth: '62ch', fontSize: '0.9375rem', lineHeight: '1.65', color: 'var(--ink-2,#b9cdc9)', textWrap: 'pretty' }}>A creator may set a share of their own yield to return to the people who are members. Here is where every unit goes when a rung matures, in the order the contract does it.</p>
              <ol style={{ margin: '1.75rem 0 0', padding: '0', listStyle: 'none', display: 'grid', gap: '0.75rem', counterReset: 'step' }}>
                <li style={{ display: 'grid', gridTemplateColumns: '2rem 1fr', gap: '0 0.875rem', alignItems: 'start' }}>
                  <span aria-hidden="true" style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', paddingTop: '0.15rem' }}>01</span>
                  <span>
                ...
                <li style={{ display: 'grid', gridTemplateColumns: '2rem 1fr', gap: '0 0.875rem', alignItems: 'start' }}>
                  <span aria-hidden="true" style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', paddingTop: '0.15rem' }}>02</span>
                  <span>
                ...
                <li style={{ display: 'grid', gridTemplateColumns: '2rem 1fr', gap: '0 0.875rem', alignItems: 'start' }}>
                  <span aria-hidden="true" style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', paddingTop: '0.15rem' }}>03</span>
                  <span>
                ...
                <li style={{ display: 'grid', gridTemplateColumns: '2rem 1fr', gap: '0 0.875rem', alignItems: 'start' }}>
                  <span aria-hidden="true" style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', paddingTop: '0.15rem' }}>04</span>
                  <span>
                ...
                <li style={{ display: 'grid', gridTemplateColumns: '2rem 1fr', gap: '0 0.875rem', alignItems: 'start' }}>
                  <span aria-hidden="true" style={{ fontFamily: 'var(--weir-mono)', fontSize: '0.8125rem', color: 'var(--crest,#8be3c6)', paddingTop: '0.15rem' }}>05</span>
                  <span>
                    <p style={{ margin: '0', fontSize: '0.9375rem', fontWeight: '600', color: 'var(--ink,#dce9e6)' }}>You claim it yourself</p>
                    <p style={{ margin: '0.25rem 0 0', fontSize: '0.9375rem', lineHeight: '1.6', color: 'var(--ink-2,#b9cdc9)', textWrap: 'pretty' }}>One transaction, whenever you like. The pool pays against your position, not against anyone&rsquo;s approval.</p>
                  </span>
                </li>
              </ol>
