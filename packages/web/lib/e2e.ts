 ...
 *
 * Pure functions, no `server-only` — the whole point is that this runs in the browser and the
 * server never sees a plaintext or a private key.
 *
...
export function decrypt(
  ...
  try {
    ...
    const plaintext = xchacha20poly1305(messageKey, fromB64(payload.nonce)).decrypt(
