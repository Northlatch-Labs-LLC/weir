...
const flaggedPublicKeyB64 = z
  .string()
  .regex(/^[A-Za-z0-9+/]{44,}={0,2}$/, 'not a flag-prefixed public key in base64');
...
export async function loadMultisigDoc(path: string): Promise<Outcome<LoadedMultisig>> {
  ...
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    return refuse('request-malformed', `the multisig document ${path} is not JSON: ${detail}`);
  }
