...
export function entryPreimage(fields, seq, prevHash) {
    const parts = [
        String(seq),
        prevHash,
        String(fields.ts),
...
export function hashEntry(fields, seq, prevHash) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (line.seq !== i) {
            return {
                intact: false,
                line: i + 1,
                reason: `line ${i + 1} declares seq ${line.seq}, and it is the ${i + 1}th line. A gap or a ` +
        ...
        if (line.prevHash !== prevHash) {
            return {
                intact: false,
                line: i + 1,
                reason: `line ${i + 1} chains from ${line.prevHash} but the previous line hashes to ${prevHash}. ` +
        ...
        if (line.hash !== expected) {
            return {
                intact: false,
                line: i + 1,
                reason: `line ${i + 1} carries hash ${line.hash} but its contents hash to ${expected}. A field ` +
...
export class AuditFile {
    ...
    #seq;
    #headHash;
    constructor(handle, path, seq, headHash) {
        this.#handle = handle;
        this.#path = path;
        this.#seq = seq;
    ...
    static async open(path) {
        ...
        if (existing.ok) {
            ...
            if (!verdict.intact) {
                return {
                    ok: false,
                    reason: `${path} already holds a broken chain at line ${verdict.line}: ${verdict.reason} ` +
