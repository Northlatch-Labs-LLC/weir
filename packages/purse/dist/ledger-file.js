...
export function outflowsOf(effects, agentAddress) {
    const wanted = normalise(agentAddress);
    const totals = new Map();
    for (const change of effects.balanceChanges) {
        if (normalise(change.address) !== wanted)
            continue;
        let amount;
        try {
...
function longestPeriodMs(policy) {
    let longest = 0;
    for (const ceiling of policy.outflowCeilings) {
        if (Number.isInteger(ceiling.periodMs) && ceiling.periodMs > longest)
            longest = ceiling.periodMs;
    }
    return longest;
}
...
export class SpendLedger {
    ...
    #entries;
    #now;
    constructor(handle, entries, now) {
        this.#handle = handle;
    ...
    static async open(args) {
        ...
        for (const line of text.split('\n')) {
            ...
            try {
                ...
                return {
                    ok: false,
                    reason: `${args.path} holds a line that is not JSON. The purse will not start on an unreadable ` +
...
//# sourceMappingURL=ledger-file.js.map