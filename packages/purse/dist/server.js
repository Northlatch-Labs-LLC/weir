...
import { createPurse } from './purse.js';
import { MAX_REQUEST_BYTES } from './protocol.js';
import { allow, refuse } from './outcome.js';
const FLAGS = ['--socket', '--policy', '--policy-sha256', '--chain', '--audit', '--spend', '--key-file'];
/**
 * Parse argv.
 *
...
export function parseServerArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (!FLAGS.includes(flag)) {
            return refuse('request-malformed', `${flag} is not a flag heron-purse takes. It takes exactly: ${FLAGS.join(' ')}. An ` +
        ...
        if (values.has(flag))
            return refuse('request-malformed', `${flag} was given twice.`);
    ...
    for (const flag of required) {
        if (!values.has(flag))
            return refuse('request-malformed', `${flag} is required.`);
    }
...
export async function startPurse(args) {
    const log = args.log ?? ((line) => process.stderr.write(`${line}\n`));
    ...
    if (key.value.signer.address !== normaliseAddress(policy.value.doc.agentAddress)) {
        return refuse('request-malformed', `the key at ${key.value.path} controls ${key.value.signer.address}, and the policy document ` +
    ...
    if (!audit.ok)
        return refuse('request-malformed', audit.reason);
    ...
    */
    ...
    const listening = await new Promise((resolve) => {
        server.once('error', (error) => resolve(refuse('request-malformed', `the socket ${args.server.socket} could not be bound: ${error.message}`)));
    ...
    log(`heron-purse: listening on ${args.server.socket} for ${purse.address} · policy ${policy.value.policyHash} ` +
...
async function serve(connection, purse) {
 ...
 */
function notifyReady(notifySocket) {
