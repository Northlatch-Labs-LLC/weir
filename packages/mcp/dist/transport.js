 ...
 */
export function agentFromReading(created) {
    const reading = created;
    if (reading?.ok === false) {
        const detail = typeof reading.failure?.detail === 'string' ? reading.failure.detail : 'no reason given';
    ...
    if (reading?.ok !== true || reading.value === null || typeof reading.value !== 'object') {
        throw new StartupRefusal(`${AGENT_PACKAGE} createAgent() did not return a Reading<Agent>. Nothing was bound, because ` +
            'binding an unrecognised shape is how a server starts with no tools and reports success.');
    }
    return portFromAgent(reading.value);
}
export function capabilitiesOf(binding) {
    const has = (name) => typeof binding.port[name] === 'function';
    const out = new Set();
    if (has('feed'))
        out.add('search');
    if (has('quote'))
...
export function agentEnvironment(env) {
    const out = {};
...
export function resolveOptions(argv, env) {
    ...
    if (wantsHttp && wantsStdio) {
        throw new StartupRefusal('both --stdio and --http were given. These are different trust models, not two ways to say ' +
    ...
    const mode = wantsHttp ? 'http' : 'stdio';
    ...
    */
    if (mode === 'http' && hasKey) {
        throw new StartupRefusal(`${ENV.key} is set and the transport is --http. This server signs nothing in HTTP mode and ` +
    ...
    if (hasKey && rawKey !== undefined && !rawKey.startsWith(SUI_PRIVATE_KEY_PREFIX)) {
        /*
          ...
          was expected.
        */
        throw new StartupRefusal(`${ENV.key} is not a Sui private key. Expected a bech32 string beginning "${SUI_PRIVATE_KEY_PREFIX}". ` +
    ...
    if (parsedBase.protocol !== 'https:' && parsedBase.hostname !== 'localhost' && parsedBase.hostname !== '127.0.0.1') {
        ...
        */
        throw new StartupRefusal(`${ENV.baseUrl} must be https (or localhost). Refusing to send signed actions over ${parsedBase.protocol}//.`);
    }
 ...
 */
export function defaultAllowedHosts(httpHost, httpPort) {
...
async function loadOptional(specifier) {
    const held = specifier;
    try {
        return (await import(held));
    }
    catch (error) {
 ...
 */
function assertSignerShape(value) {
    ...
    if (typeof candidate.signPersonalMessage !== 'function')
        return null;
 ...
 */
async function probeSigner(signer) {
    try {
        const result = await signer.signPersonalMessage(probeBytes());
 ...
 */
export async function openWeir(options) {
    const agentModule = await loadOptional(AGENT_PACKAGE);
    if (agentModule === null) {
        throw new StartupRefusal(`${AGENT_PACKAGE} could not be loaded, so this server has nothing to talk to weir with.`);
    }
    ...
    */
    ...
    if (options.secretKey !== null) {
        try {
            ...
            void error;
            throw new StartupRefusal(`${ENV.key} is set but could not be decoded as a bech32 Ed25519 Sui private key ` +
 ...
 */
export function loadPolicyDoc(text, signerAddress) {
    let parsed;
    try {
    ...
    if (parsed === null || typeof parsed !== 'object')
        return { ok: false, reason: 'the policy file is not an object' };
    const doc = parsed;
    if (doc['version'] !== 1)
        return { ok: false, reason: `the policy file has version ${JSON.stringify(doc['version'])}; this server reads version 1` };
    ...
    for (const field of ['outflowCeilings', 'allowedTargets', 'allowedTypeArguments', 'allowedRecipients', 'allowedObjects']) {
        if (!Array.isArray(doc[field]))
            return { ok: false, reason: `the policy file lacks the ${field} list` };
    }
 ...
 */
async function bindPolicy(options, signer) {
    ...
    if (!loaded.ok)
        throw new StartupRefusal(`${ENV.policy}: ${loaded.reason}`);
    ...
    */
    ...
    return {
        factory: (client) => make({ inner: signer.signer, policy, client, ledger }),
    };
}
async function bindSigner(options) {
    if (options.secretKey === null)
        return { kind: 'none' };
    const signerModule = await loadOptional(SIGNER_PACKAGE);
    if (signerModule === null)
        return { kind: 'none' };
    ...
    */
    const reading = open(options.secretKey);
    if (reading.ok !== true) {
        throw new StartupRefusal(`${SIGNER_PACKAGE} could not open the signing key: ${reading.failure?.detail ?? 'no detail given'} ` +
            '(the key itself has not been logged).');
    }
    const shaped = assertSignerShape(reading.value);
    if (shaped === null) {
        throw new StartupRefusal(`${SIGNER_PACKAGE} returned something that is not a Signer. Expected { address, scheme, ` +
 ...
 */
export async function serveHttp(newServer, options) {
    ...
    */
    if (req.headers.cookie !== undefined) {
        respondJson(res, 400, {
            ...
            detail: 'this endpoint accepts no cookies and issues none. There is no session here to carry: ' +
 ...
 */
function asTransport(transport) {
    return transport;
}
/**
