...
import { ENV, capabilitiesOf, log, openWeir, resolveOptions, serveHttp, serveStdio, StartupRefusal, } from './transport.js';
const SERVER_INFO = {
    name: 'weir-mcp',
    version: '1.0.0',
    title: 'weir.social',
 ...
 */
function announce(binding, options) {
    ...
    switch (binding.signer.kind) {
        case 'none':
            ...
            break;
        case 'read-only':
            log(`mode=${options.mode} base=${options.baseUrl} signing=READ-ONLY as ${binding.signer.signer.address} ` +
                `(scheme=${binding.signer.signer.scheme}; it has no signTransaction, so it cannot move value)`);
            break;
        case 'signing':
            log(`mode=${options.mode} base=${options.baseUrl} signing=YES as ${binding.signer.signer.address} ` +
