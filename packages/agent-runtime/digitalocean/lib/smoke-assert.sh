...
# nobody has built is a block nobody has ever run. Every path here has a HERON_* seam so the whole
...
# read of step 6:
#
#   1. /srv/heron/chain.json is readable BY UID 10002 and parses. heron-purse.service runs as
#      `purse` and reads that file at start; at root:root the signer refuses to start and, through
#      heron-beat.service's Requires=, no beat ever runs.
...
#   3. root has no ~/.docker. heron-beat.service runs as root under ProtectHome=yes, which hides
...
PURSE_UID="${HERON_PURSE_UID:-10002}"
DOCKER_BIN="${HERON_DOCKER_BIN:-/usr/bin/docker}"
SYSTEMD_RUN="${HERON_SYSTEMD_RUN:-systemd-run}"
ROOT_HOME="${HERON_ROOT_HOME:-/root}"
...
$SUDO test -f "$CHAIN" || fail "$CHAIN does not exist; heron-purse.service names it as --chain and reads it at start"
...
DOCKER_VERSION="$($SUDO "$SYSTEMD_RUN" --pipe --quiet \
  ...
  || fail "the docker socket is NOT reachable from a unit under ProtectSystem=strict and ProtectHome=yes: $DOCKER_VERSION. heron-beat.service carries both, and its own comment names the one-line fallback (ReadWritePaths=/run/docker.sock)."
...
if $SUDO test -e "$ROOT_HOME/.docker"; then
  fail "$ROOT_HOME/.docker exists, and heron-beat.service runs as root under ProtectHome=yes: the beat would silently not see a client config every hand-run docker command does see"
fi
