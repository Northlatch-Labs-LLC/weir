...
// HERON_NO_NETWORK=1, which is the only way deploy-droplet.sh will run it at all.
//
...
const PURSE_SYSTEMD = path.join(PKG_DIR, '..', 'purse', 'systemd');

function tmp(prefix) {
  return mkdtempSync(path.join(os.tmpdir(), `heron-${prefix}-`));
}
...
test('B1: --plan prints the purse row so the Master reads the account before it exists', () => {
  ...
  assert.match(plan, /heron\s+gid 10001 \/ uid 10001/, '--plan never names the heron account');
});

test('B1: the purse account is not in the heron group, and the post-boot check asserts it', () => {
  const yaml = readCloudInit();
  assert.doesNotMatch(yaml, /usermod[^\n]*-aG[^\n]*purse/, 'nothing may add purse to another group');
  assert.match(
    readFileSync(POST_BOOT, 'utf8'),
    /id -nG purse[\s\S]*?grep -qx heron/,
    'the post-boot check must refuse if purse is in the heron group',
  );
});
...
const NOT_CLOUD_INIT = new Map([
  ['/run/heron', 'RuntimeDirectory=heron creates it at unit start; /run is a tmpfs cloud-init cannot pre-create'],
  ['/run/heron/purse.sock', 'the purse creates and chmods its own socket at 0660 purse:heron'],
  ['/run/docker.sock', "the docker.io package's own unit creates it"],
  ['/usr/local/sbin/heron-watchdog', 'installed by deploy-droplet.sh install_host_units, 0755 root:root'],
...
test('B2: the hot key\'s home and the signer\'s paths are created, at the mode they must have', () => {
  const created = cloudInitInstalledPaths();
  const expect = [
    ['/srv/heron/keys', '0700', 'purse', 'purse'],
    ['/srv/heron/purse', '0750', 'purse', 'purse'],
    ['/srv/heron/policy', '0755', 'root', 'root'],
    ['/srv/heron/runs', '2770', 'root', 'heron'],
    ['/srv/heron/runs/archive', '2770', 'root', 'heron'],
    ['/srv/heron/state', '2770', 'root', 'heron'],
    ['/etc/heron/creds', '0700', 'root', 'root'],
...
// ---------------------------------------------------------------------------
// B3. One credential directory: /etc/heron/creds, everywhere.
// ---------------------------------------------------------------------------

test('B3: no unit, script, config or document names any credential directory but /etc/heron/creds', () => {
  const files = [];
  const walk = (dir) => {
  ...
  for (const file of files) {
    ...
    text.split('\n').forEach((line, index) => {
      ...
      // why it is gone; a comment that says "/srv/heron/creds" and nothing else would pass, so the
...
test('B3: the beat unit and the purse unit load their credentials from the same directory', () => {
  const beat = readFileSync(path.join(PURSE_SYSTEMD, 'heron-beat.service'), 'utf8');
  const purse = readFileSync(path.join(PURSE_SYSTEMD, 'heron-purse.service'), 'utf8');
  const alert = readFileSync(path.join(DO_DIR, 'systemd', 'heron-alert@.service'), 'utf8');
  assert.match(beat, /LoadCredentialEncrypted=openrouter:\/etc\/heron\/creds\/openrouter\.cred/);
  assert.match(purse, /LoadCredentialEncrypted=heron-hot:\/etc\/heron\/creds\/heron-hot\.cred/);
  assert.match(alert, /LoadCredentialEncrypted=mail-key:\/etc\/heron\/creds\/mail-key\.cred/);
});

test('B3: --seal writes into /etc/heron/creds and --plan and the README say the same', () => {
  const dryRun = spawnSync('bash', [DEPLOY_SCRIPT, '--seal', 'mail-key', '--dry-run'], { encoding: 'utf8' });
  assert.equal(dryRun.status, 0, dryRun.stderr);
  assert.match(dryRun.stdout, /\/etc\/heron\/creds\/mail-key\.cred/);
  assert.match(runPlan(), /\/etc\/heron\/creds/);
  assert.match(readFileSync(path.join(DO_DIR, 'README.md'), 'utf8'), /\/etc\/heron\/creds/);
});
...
function hostFixture() {
  ...
  write('sudo', `#!/usr/bin/env bash\nexec env HERON_FIXTURE_PRIVILEGED=1 "$@"\n`);
  write(
    'cloud-init',
    `#!/usr/bin/env bash
if [ "\${HERON_FIXTURE_PRIVILEGED:-}" != "1" ]; then
...
`,
  ...
  write(
    ...
    `#!/usr/bin/env bash
case "$1:$2" in
  passwd:10001) echo "heron:x:10001:10001::/nonexistent:/usr/sbin/nologin" ;;
  group:10001)  echo "heron:x:10001:" ;;
...
`,
  ...
  mkdirSync(sshdConfD);
  writeFileSync(path.join(sshdConfD, '00-heron.conf'), 'PasswordAuthentication no\n', 'utf8');
  writeFileSync(path.join(sshdConfD, '50-cloud-init.conf'), 'PasswordAuthentication yes\n', 'utf8');

  // The /srv/heron, /etc/heron and /var/lib/heron layout, at the modes cloud-init claims.
  const srv = path.join(dir, 'srv-heron');
  const etc = path.join(dir, 'etc-heron');
  const varlib = path.join(dir, 'var-heron');
  const make = (p, mode) => {
  ...
  const env = {
    ...
    HERON_SSHD_BIN: path.join(bin, 'sshd'),
    HERON_DEBIAN_VERSION_FILE: path.join(dir, 'debian_version'),
    HERON_SSHD_CONFIG_D: sshdConfD,
    HERON_SRV_ROOT: srv,
    HERON_ETC_ROOT: etc,
  ...
  delete env.HERON_FIXTURE_PRIVILEGED;
...
test('B4: the hardening keywords are read out of sshd -T, so a "yes" fails the deploy', () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      `#!/usr/bin/env bash
if [ "\${HERON_FIXTURE_PRIVILEGED:-}" != "1" ]; then exit 1; fi
printf 'passwordauthentication no\\npermitrootlogin yes\\nkbdinteractiveauthentication no\\n'
`,
      'utf8',
    );
...
test('B4: a plaintext credential left under /srv/heron fails the post-boot check', () => {
  const fixture = hostFixture();
  try {
    writeFileSync(path.join(fixture.srv, 'keys', 'heron-hot.key'), 'NOT-A-REAL-VALUE\n', 'utf8');
    const result = spawnSync('bash', [POST_BOOT], { encoding: 'utf8', env: fixture.env });
    assert.notEqual(result.status, 0);
...
test('B4: every privileged command in the post-boot file carries sudo, and sshd by absolute path', () => {
  ...
  assert.match(text, /SSHD_BIN="\$\{HERON_SSHD_BIN:-\/usr\/sbin\/sshd\}"/, 'sshd must default to its absolute path');
...
test('B6/A1: HERON_NO_NETWORK=1 with no stub directory refuses rather than reaching the network', () => {
  const fixture = createStubs();
  try {
    const env = { ...fixture.env };
    delete env.HERON_STUB_DIR;
    const result = spawnSync('bash', [DEPLOY_SCRIPT, '--create'], { encoding: 'utf8', env });
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /there is no HERON_STUB_DIR to stand in for it/);
  } finally {
    rmSync(fixture.dir, { recursive: true, force: true });
  }
});
...
function sealFixture() {
  ...
  writeFileSync(
    ...
    `#!/usr/bin/env bash
printf '%s\\n' "$@" > "$HERON_TEST_SSH_ARGV"
cat > "$HERON_TEST_SSH_STDIN"
exit \${HERON_TEST_SSH_EXIT:-0}
`,
    'utf8',
  );
  ...
  return {
    ...
    env: {
      ...
      HERON_PILE: pile,
      HERON_TEST_SSH_ARGV: argvFile,
      HERON_TEST_SSH_STDIN: stdinFile,
    },
  };
}
...
test('A2: --seal refuses a name that is not ^[a-z][a-z0-9-]{0,31}$, before it reads anything', () => {
  ...
  for (const name of bad) {
    const result = spawnSync('bash', [DEPLOY_SCRIPT, '--seal', name, '--dry-run'], {
      encoding: 'utf8',
      env: { ...process.env, HERON_DEPLOY_CONFIRMED: '1' },
    });
...
test('A2: --seal accepts the names the ledger actually uses', () => {
  for (const name of ['mail-key', 'openrouter', 'heron-hot', 'heron-policy']) {
    ...
    assert.match(result.stdout, new RegExp(`--name=${name} - /etc/heron/creds/${name}\\.cred`));
  }
});
...
test('A3: --seal pipes the credential on STDIN and writes no plaintext file on either end', () => {
  ...
  try {
    ...
    assert.match(argv, /sudo systemd-creds encrypt --with-key=host --name=mail-key - \/etc\/heron\/creds\/mail-key\.cred/);
...
test('A3: --seal fails if the far end fails -- set -o pipefail, not a silent success', () => {
  ...
  try {
    const result = spawnSync('bash', [DEPLOY_SCRIPT, '--seal', 'mail-key'], {
      encoding: 'utf8',
      env: { ...fixture.env, HERON_TEST_SSH_EXIT: '3' },
    });
...
test('A3: --seal refuses without the Master\'s word, and refuses a pile file that is not 0600', () => {
  ...
  try {
    ...
    delete noWord.HERON_DEPLOY_CONFIRMED;
...
test('A4: the alert names WHICH thing happened, so a recovery does not arrive worded as a failure', () => {
  ...
  try {
    ...
    const result = spawnSync('python3', [path.join(DO_DIR, 'bin', 'heron-alert'), 'watchdog', '--dry-run'], {
      encoding: 'utf8',
      env: { ...process.env, HERON_ALERTS_FILE: f.alerts },
    });
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /subject: Heron recovered/);
  } finally {
    rmSync(f.dir, { recursive: true, force: true });
  }
});
...
test('A5: heron-alert@.service carries the hardening set', () => {
  ...
  for (const directive of [
    ...
    assert.ok(svc.includes(directive), `heron-alert@.service is missing ${directive}`);
  }
...
test('A6: run-flags, cloud-init, the beat unit and the purse source all agree on the intent path', () => {
  ...
  const yaml = readCloudInit();
  const beatUnit = readFileSync(path.join(PURSE_SYSTEMD, 'heron-beat.service'), 'utf8');
  ...
  assert.doesNotMatch(runFlags, /source=\/srv\/heron\/intents/, 'run-flags still mounts /srv/heron/intents');
  assert.doesNotMatch(yaml, /\/srv\/heron\/intents/, 'cloud-init still creates /srv/heron/intents');
  assert.doesNotMatch(beatUnit, /\/srv\/heron\/intents/);
...
test('A6: the container has exactly two writable mounts, runs/ and state/', () => {
  ...
  assert.deepEqual(writable.sort(), ['/srv/heron/runs', '/srv/heron/state']);
  const readonly = mounts.filter((m) => m[3] === 'true').map((m) => m[1]);
  assert.deepEqual(readonly, ['/srv/heron/image.env']);
});
...
test('A7: 250 per-beat DIRECTORIES fall to 200, the other 50 archived whole and readable', () => {
  ...
  try {
    ...
    const result = spawnSync(RETENTION, [], { encoding: 'utf8', env: { ...process.env, HERON_RUNS_DIR: dir } });
  ...
  try {
    ...
    const result = spawnSync(RETENTION, [], {
      ...
      env: {
        ...process.env,
        HERON_RUNS_DIR: dir,
        HERON_RETENTION_KEEP: '200',
        HERON_RETENTION_CEILING_BYTES: String(4096 * 10),
      },
    });
    assert.equal(result.status, 0, result.stderr);
    const live = readdirSync(dir).filter((n) => n !== 'archive');
...
test('A7: a mixed directory of beat directories and plain logs is handled as one ordering', () => {
  ...
  try {
    ...
    const result = spawnSync(RETENTION, [], {
      encoding: 'utf8',
      env: { ...process.env, HERON_RUNS_DIR: dir, HERON_RETENTION_KEEP: '2' },
    });
    assert.equal(result.status, 0, result.stderr);
    const live = readdirSync(dir).filter((n) => n !== 'archive');
...
test('N8: archive/ is created 2770, not whatever the umask leaves', () => {
  ...
  try {
    ...
    try {
      ...
      const result = spawnSync(RETENTION, [], {
        encoding: 'utf8',
        env: { ...process.env, HERON_RUNS_DIR: dir, HERON_RETENTION_KEEP: '0' },
      });
      assert.equal(result.status, 0, result.stderr);
      const mode = statSync(path.join(dir, 'archive')).mode & 0o7777;
      assert.equal(mode.toString(8), '2770', `archive/ is ${mode.toString(8)}, so the heron group cannot read it`);
    } finally {
...
test('N8: cloud-init also creates runs/archive at 2770 root:heron', () => {
  const row = cloudInitInstalledPaths().get('/srv/heron/runs/archive');
  assert.ok(row, 'cloud-init never creates runs/archive');
  assert.equal(row.mode, '2770');
  assert.equal(row.owner, 'root');
  assert.equal(row.group, 'heron');
});

// ---------------------------------------------------------------------------
// A8. heron-beat.service's "only writable paths" is enforced by something.
// ---------------------------------------------------------------------------

test('A8: heron-beat.service sets ProtectSystem=strict, so ReadWritePaths carves out of something', () => {
  const svc = readFileSync(path.join(PURSE_SYSTEMD, 'heron-beat.service'), 'utf8');
...
test('N1: there is one shell account on this host, not two sharing one key', () => {
  const yaml = readCloudInit();
  assert.doesNotMatch(yaml, /- name: heron-ops/, 'heron-ops was a second door on the same key, used by nothing');
...
test('N5: the tarball is checksummed ON THE HOST before docker build reads it', () => {
  ...
  const dockerBuild = build.indexOf('docker build -t heron:local');
...
test('N6: the mail-key row says the mode systemd-creds actually writes', () => {
  const plan = runPlan();
  assert.match(plan, /mail-key.*\/etc\/heron\/creds\/mail-key\.cred\s+0600/);
...
test('N7: a logrotate config exists, bounds the JSONL sinks only, and is installed by the deploy', () => {
  ...
  const script = readFileSync(DEPLOY_SCRIPT, 'utf8');
  assert.match(script, /mv \/tmp\/heron\.logrotate \/etc\/logrotate\.d\/heron/);
  assert.match(script, /logrotate --debug \/etc\/logrotate\.d\/heron/, 'the config must be parsed on the host');
...
test('the on-host image build points the docker client away from /root/.docker, which the beat unit cannot see', () => {
  ...
  assert.ok(build.indexOf('export DOCKER_CONFIG') < build.indexOf('docker build -t heron:local'), 'DOCKER_CONFIG must be set before docker build runs');
